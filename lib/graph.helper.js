 ...
 * @typedef {{id: string, name: string, folders: MsFolder[], files: MsFile[]}} MsFolder
 */
...
module.exports.uploadValidFolders = async function (folders) {
    let validFolders = await getValidFolders(folders);
    let validFiles = await getFilesOfFolders(validFolders);
 ...
 * @returns {Promise<MsFolder[]>} array of valid folders whos files need to be uploaded
 */
async function getValidFolders(folders, level = 0, foldersToUpload = []) {
    /** TODO - possible optimization: the getFunctionByLevel calls can easily be grouped together 
     * to reduce overhead of multiple single calls (by replacing with fewer grouped 
     * calls)
    */
    if (!folders.length) {
    ...
    for (let folder of folders) {
        ...
        await getValidFolders(folder.folders, level + 1, foldersToUpload)
